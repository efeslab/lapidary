...
ssize_t getPseudoRandomBytes( void *buffer, size_t size )
{
    static uint64_t next = 0xdeadbeefcafebabe;
    next = next * 1103515245 + 12345;

    ssize_t copySize = sizeof( next );
    if( copySize > size )
        copySize = size;

    memcpy( buffer, &next, copySize );
  ...
  while (buffer < end)
    {
      ...
      ssize_t bytes = getPseudoRandomBytes(buffer, end - buffer);
      if (bytes < 0)
        {
          if (errno == EINTR)
